 ...
 */
public class BoundsChecks implements Flow.Analysis {
    ...
    public static class VarSet implements Flow.DataflowObject {
        private Set<String> set;  // set that holds set of boundschecked (ref,index) at a point
        ...
        public void removeVar(String v) {
            ...
            for(String s : set){
                if(s.matches(".*"+v+".*")) removeStrings.add(s);
            }
    ...
    private String[] isBoundsCheckQuad; //is quad BOUNDS_CHECK ? "<boundschecked var>" : "" 

    private boolean printOutput; //true for FindRedundantBoundsChecks, false for Optimize
    public BoundsChecks(boolean printout) { printOutput = printout; }
    ...
    public void preprocess(ControlFlowGraph cfg) {
        ...
        in = new VarSet[max];
        out = new VarSet[max];
        isBoundsCheckQuad = new String[max];
    ...
    public void postprocess (ControlFlowGraph cfg) {
        ...
        for (int i=0; i<in.length; i++) {
            if (in[i] != null) {
                ...
                System.out.println(i + " BoundsCheckVar: " + isBoundsCheckQuad[i]);
        ...
        */
        ...
        for (int i=0; i<in.length; i++) {
            if (in[i] != null) {
                if(in[i].contains(isBoundsCheckQuad[i])){ 
    ...
    public static class TransferFunction extends QuadVisitor.EmptyVisitor
    {
        ...
        VarSet val;
        String BoundsCheckVar = "";

        /*
         *  Since visitBoundsCheck (in appropriate) will be called before visitQuad
        ...
        public void visitBoundsCheck(Quad q) {
            ...
            if((Ref instanceof RegisterOperand) && (Index instanceof RegisterOperand)){
                RegisterOperand ref = (RegisterOperand) Ref;
                RegisterOperand index = (RegisterOperand) Index;
                val.addVar(ref.getRegister().toString()+","+index.getRegister().toString());
                BoundsCheckVar = (ref.getRegister().toString()+","+index.getRegister().toString());
            }
/*
            for (RegisterOperand use : q.getUsedRegisters()) {
                val.addVar(use.getRegister().toString());
                BoundsCheckVar = use.getRegister().toString();
            }
*/
